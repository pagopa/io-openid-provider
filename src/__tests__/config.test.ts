...
describe("Config", () => {
  ...
  describe("given an invalid configuration", () => {
    it("should fail because hostname is missing", () => {
      ...
      const validLogLevel =
        `"debug" | "trace" | "error" | "info" | "fatal" | "warn"`

      const expected = E.left(
        FS.concat(
          FS.concat(
            FS.of(DE.key("LOG_LEVEL", DE.required, FS.of(DE.leaf(undefined, validLogLevel)))),
            FS.of(DE.key("SERVER_HOSTNAME", DE.required, FS.of(DE.leaf(undefined, "string")))),
          ),
          FS.of(DE.key("SERVER_PORT", DE.required, FS.of(DE.leaf(NaN, "Not a number")))),
        )
      )
